...
<div class="fade-in">
    ...
    <form action="{{ route('admin.task-templates.update', $taskTemplate) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-lg-8">
                <div class="dashboard-card mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                               value="{{ old('title', $taskTemplate->title) }}" required>
                        @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control @error('description') is-invalid @enderror"
                                  rows="3">{{ old('description', $taskTemplate->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="service_id" class="form-select @error('service_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($services as $service)
                                        <option value="{{ $service->id }}" {{ old('service_id', $taskTemplate->service_id) == $service->id ? 'selected' : '' }}>
                                            {{ $service->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="service_stage_id" class="form-select @error('service_stage_id') is-invalid @enderror">
                                    ...
                                    @foreach($stages as $stage)
                                        <option value="{{ $stage->id }}" {{ old('service_stage_id', $taskTemplate->service_stage_id) == $stage->id ? 'selected' : '' }}>
                                            {{ $stage->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="dashboard-card mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="required_skills[]" class="form-select @error('required_skills') is-invalid @enderror" multiple size="5">
                            @foreach($skills as $skill)
                                <option value="{{ $skill->id }}" {{ in_array($skill->id, old('required_skills', $taskTemplate->required_skills ?? [])) ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="required_expertise_level" class="form-select @error('required_expertise_level') is-invalid @enderror">
                            ...
                            <option value="junior" {{ old('required_expertise_level', $taskTemplate->required_expertise_level) == 'junior' ? 'selected' : '' }}>Junior</option>
                            <option value="mid" {{ old('required_expertise_level', $taskTemplate->required_expertise_level) == 'mid' ? 'selected' : '' }}>Mid-Level</option>
                            <option value="senior" {{ old('required_expertise_level', $taskTemplate->required_expertise_level) == 'senior' ? 'selected' : '' }}>Senior</option>
                            <option value="lead" {{ old('required_expertise_level', $taskTemplate->required_expertise_level) == 'lead' ? 'selected' : '' }}>Lead</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="dependencies[]" class="form-select @error('dependencies') is-invalid @enderror" multiple size="4">
                            @foreach($templates as $template)
                                <option value="{{ $template->id }}" {{ in_array($template->id, old('dependencies', $taskTemplate->dependencies->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $template->title }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="dashboard-card mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="priority" class="form-select @error('priority') is-invalid @enderror" required>
                            <option value="low" {{ old('priority', $taskTemplate->priority) == 'low' ? 'selected' : '' }}>Low</option>
                            <option value="medium" {{ old('priority', $taskTemplate->priority) == 'medium' ? 'selected' : '' }}>Medium</option>
                            <option value="high" {{ old('priority', $taskTemplate->priority) == 'high' ? 'selected' : '' }}>High</option>
                            <option value="urgent" {{ old('priority', $taskTemplate->priority) == 'urgent' ? 'selected' : '' }}>Urgent</option>
                        </select>
                        @error('priority')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="estimated_hours" class="form-control @error('estimated_hours') is-invalid @enderror"
                               value="{{ old('estimated_hours', $taskTemplate->estimated_hours) }}" min="1">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="default_duration_days" class="form-control @error('default_duration_days') is-invalid @enderror"
                               value="{{ old('default_duration_days', $taskTemplate->default_duration_days) }}" min="1">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="sort_order" class="form-control @error('sort_order') is-invalid @enderror"
                               value="{{ old('sort_order', $taskTemplate->sort_order) }}" min="0">
                        @error('sort_order')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input type="checkbox" name="requires_review" class="form-check-input" id="requires_review"
                                   value="1" {{ old('requires_review', $taskTemplate->requires_review) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input type="checkbox" name="is_active" class="form-check-input" id="is_active"
                                   value="1" {{ old('is_active', $taskTemplate->is_active) ? 'checked' : '' }}>
