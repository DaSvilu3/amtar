...
<div class="fade-in">
    ...
    <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" class="form-control @error('progress') is-invalid @enderror"
                           id="progress" name="progress" value="{{ old('progress', $project->progress ?? 0) }}" min="0" max="100">
                    @error('progress')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="dashboard-card mb-4">
                    ...
                    <select class="form-select @error('main_service_id') is-invalid @enderror"
                        ...
                        @foreach($mainServices as $mainService)
                            <option value="{{ $mainService->id }}"
                                    {{ old('main_service_id', $project->main_service_id) == $mainService->id ? 'selected' : '' }}
                                    data-has-subs="{{ $mainService->subServices->count() > 0 ? '1' : '0' }}">
                                {{ $mainService->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-3">
                <h6 class="mb-3"><i class="fas fa-list-check me-2"></i>Current Project Services</h6>
                <div id="current-services-list" class="service-list">
                    @php
                        $currentServices = $project->services()->with('service.serviceStage')->orderBy('sort_order')->get();
                        $groupedServices = $currentServices->groupBy('serviceStage.name');
                    @endphp
                    @forelse($groupedServices as $stageName => $stageServices)
                        <div class="service-stage">
                            ...
                            @foreach($stageServices as $projectService)
                                <div class="service-item {{ $projectService->is_from_package ? 'from-package' : 'custom' }}">
                                    <input type="checkbox" checked disabled class="form-check-input">
                                    <span>{{ $projectService->service->name }}</span>
                                    <span class="service-badge {{ $projectService->is_from_package ? 'badge-package' : 'badge-custom' }} ms-auto">
                    ...
                    @empty
                        <p class="text-muted text-center my-3">No services selected</p>
                    @endforelse
                </div>
            </div>

            <!-- Package Services Display (for new selection) -->
            <div id="package-services-section" class="mt-3" style="display: none;">
                <h6 class="mb-3"><i class="fas fa-list-check me-2"></i>New Package Services</h6>
        ...
        <!-- Documents -->
        <div class="dashboard-card mb-4">
            <h5 class="mb-3"><i class="fas fa-file-upload me-2"></i>Update Documents</h5>
            <p class="text-muted small">Upload new documents or replace existing ones. Leave empty to keep current documents.</p>

            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="document_mulkiya" class="form-label">Project Mulkiya (Property Title Deed)</label>
                ...
                <div class="col-md-6 mb-3">
                    <label for="document_kuroki" class="form-label">Project Kuroki (Project Sketch/Plan)</label>
...
let packageServices = [];
const initialMainServiceId = {{ $project->main_service_id ?? 'null' }};
const initialSubServiceId = {{ $project->sub_service_id ?? 'null' }};
const initialPackageId = {{ $project->service_package_id ?? 'null' }};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (initialMainServiceId) {
        initializeExistingData();
    }

    async function initializeExistingData() {
        try {
            // Load sub-services if main service has them
            const subServicesResponse = await fetch(`/admin/api/services/sub-services/${initialMainServiceId}`);
            ...
            if (subServices.length > 0) {
                subServices.forEach(subService => {
                    ...
                    option.selected = subService.id == initialSubServiceId;
            ...
            // Load packages
            await loadPackages(initialMainServiceId, initialSubServiceId, initialPackageId);
        } catch (error) {
    ...
    async function loadPackages(mainServiceId, subServiceId, selectedPackageId = null) {
        try {
            ...
            // If there's a selected package, trigger the change event
