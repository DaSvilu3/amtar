...
<div class="fade-in">
    ...
    <div class="dashboard-card">
        <form action="{{ route('admin.services.sub.store') }}" method="POST">
            @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('main_service_id') is-invalid @enderror"
                            ...
                            @foreach($mainServices as $main)
                                <option value="{{ $main->id }}" {{ old('main_service_id', request('main_service_id')) == $main->id ? 'selected' : '' }}>
                                    {{ $main->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="d-flex justify-content-end gap-2">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-2"></i>Create Sub Service
                </button>
            </div>
        </form>
    </div>
