...
<div class="fade-in">
    ...
    <div class="dashboard-card">
        <form action="{{ route('admin.services.services.update', $service) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('service_stage_id') is-invalid @enderror"
                            ...
                            @foreach($stages as $stage)
                                <option value="{{ $stage->id }}" {{ old('service_stage_id', $service->service_stage_id) == $stage->id ? 'selected' : '' }}>
                                    {{ $stage->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="documents-container">
                            @php
                                $documents = old('required_documents', $service->required_documents ?? []);
                            @endphp
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="is_optional" name="is_optional" value="1"
                                   {{ old('is_optional', $service->is_optional) ? 'checked' : '' }}>
